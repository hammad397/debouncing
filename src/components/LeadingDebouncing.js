...
const LeadingDebouncing = ({show, view, subHeading}) => {
  ...
  return (
    ...
    {show? <input type="text" placeholder='Suggestions for...' onInput={decoratedDebounce}/> :
    <button onClick={decoratedDebounce}>Add to cart</button>} 
    <h1 className='sub-heading'>{subHeading}</h1>
    {text? <p>{text}</p> : <p style={{color: 'grey'}}>{items.length < 1 && view}</p>}
    {!show && items.length !== 0 && items.map((item) => <p>{item}</p>)}
    </div>
  )
}
